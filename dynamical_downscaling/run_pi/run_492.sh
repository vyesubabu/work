#!/bin/bash
export TIME_SOFT=/home2_hn/xuwq/github/work/dynamical_downscaling/advancetime_noleap.py
export TIME_SOFT2=/home/xuwq/wrf_3.7.1/WRFDA/var/build/da_advance_time.exe
export RUN_DIR=/home2_hn/xuwq/no_leap_year/WRFV3/run
################################################
case_name="pi"
date_start="1991123100"
date_end="1992123100"
time_interval="120"         
run_day="6"
run_hour="00"
namelist_dir=/home2_hn/xuwq/github/work/dynamical_downscaling
met_dir=/home2_hn/xuwq/no_leap_year/WPS_PI
...
function run_real () {
    ...
    cd $real_output_dir
    start_time_pi='0'`expr $start_time - 1500000000`
    end_time_pi='0'`expr $end_time - 1500000000`

    cp $namelist_file ./namelist.input
    local y_start="$(echo $start_time_pi | cut -c 1-4 )"
    local m_start="$(echo $start_time_pi | cut -c 5-6 )"
    local d_start="$(echo $start_time_pi | cut -c 7-8 )"
    local h_start="$(echo $start_time_pi | cut -c 9-10)"
    local y_end="$(echo $end_time_pi | cut -c 1-4 )"
    local m_end="$(echo $end_time_pi | cut -c 5-6 )"
    local d_end="$(echo $end_time_pi | cut -c 7-8 )"
    local h_end="$(echo $end_time_pi | cut -c 9-10)"
    ...
    while [ $curr_time != $end_time ]; do
        curr_time_pi='0'`expr $curr_time - 1500000000`
        curr_w=$(${TIME_SOFT2} $curr_time_pi 0 -w)
        curr_w=${zero}${curr_w:1}
...
function run_wrf () {
    ...
    cd $wrf_output_dir
    cp /home2_hn/xuwq/github/work/dynamical_downscaling/job_10 .
    qsub job_10
